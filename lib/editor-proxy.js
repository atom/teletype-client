...
class EditorProxy {
  ...
  updateSelections (selections = {}, preserveTetherState = false) {
    ...
    if (!preserveTetherState && this.tetherState === RETRACTED) {
      ...
      if (localCursorPosition && !pointsEqual(localCursorPosition, tetherPosition)) {
        ...
        this.updateSelections(this.localSelections)
      }
    }
  }
  ...
  tetherToSiteId (tetherSiteId) {
    ...
    if (this.delegate) this.delegate.updateTether(this.getTetherPosition())
  }
        ...
        if (siteId !== this.siteId) {
          ...
          for (let layerId in layersById) {
            ...
            if (this.selectionLayerIdsBySiteId.get(siteId) === layerId) {
              ...
              if (siteId === this.tetherSiteId) this.retractOrDisconnectTether()
            }
          }
        }
      }
  ...
  retractOrDisconnectTether () {
    ...
    this.delegate.updateTether(tetherPosition)
    this.tetherState = RETRACTED
    this.updateSelections(this.localSelections, true)
  }

  didScroll () {
    if (this.tetherSiteId != null && this.delegate && !this.delegate.isPositionVisible(this.getTetherPosition())) {
      this.tetherState = DISCONNECTED
    }
  }
