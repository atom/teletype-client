...
class EditorProxy {
  ...
  updateSelections (selections = {}, preserveFollowState = false) {
    ...
    for (const id in selections) {
      ...
      if (selection) {
        this.localSelections[id] = selection
      } else {
        delete this.localSelections[id]
      }
    }
    ...
    if (!preserveFollowState && this.resolveFollowState() === FollowState.RETRACTED) {
      const localCursorPosition = this.cursorPositionForSiteId(this.siteId)
      const tetherLeaderPosition = this.resolveLeaderPosition()
      if (localCursorPosition && tetherLeaderPosition && !pointsEqual(localCursorPosition, tetherLeaderPosition)) {
  ...
  setFollowState (newState, leaderId) {
    if (leaderId == null) {
      leaderId = this.tethersByFollowerId.get(this.siteId).leaderId
    }

    const previousResolvedState = this.resolveFollowState()

    this.tethersByFollowerId.set(this.siteId, {leaderId, state: newState})

    if (previousResolvedState === FollowState.RETRACTED || this.resolveFollowState() === FollowState.RETRACTED) {
      this.updateSelections(this.localSelections, true)
    }

    if (this.delegate) {
      this.delegate.updateTether(this.resolveFollowState(), this.resolveLeaderPosition())
    }

    const tetherUpdateMessage = new Messages.EditorProxyUpdate.TetherUpdate()
    tetherUpdateMessage.setFollowerSiteId(this.siteId)
    tetherUpdateMessage.setLeaderSiteId(leaderId)
  ...
  didScroll () {
    const tetherPosition = this.resolveLeaderPosition()
      ...
      this.selectionLayerIdsBySiteId.forEach((_, siteId) => {
        if (siteId !== this.siteId) {
          if (this.resolveFollowState(siteId) === FollowState.RETRACTED) {
            activePositions[siteId] = this.resolveLeaderPosition(siteId)
          } else {
            activePositions[siteId] = this.cursorPositionForSiteId(siteId)
          }
        }
      })
  ...
  siteDidDisconnect (siteId) {
    if (siteId === this.resolveLeaderSiteId()) {
      this.unfollow()
    }
    if (this.delegate) {
  ...
  receiveTetherUpdate (tetherUpdate) {
    ...
    this.updateActivePositions()
    if (this.delegate && this.resolveLeaderSiteId() !== oldResolvedLeaderSiteId) {
      this.delegate.updateTether(this.resolveFollowState(), this.resolveLeaderPosition())
    }
  }
}
