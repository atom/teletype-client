...
class EditorProxy {
  ...
  constructor ({id, bufferProxy, selectionLayerIdsBySiteId, selections, router, siteId, tetherBreakPeriod, didDispose}) {
    this.id = id
    this.siteId = siteId
  ...
  updateTether () {
    ...
    if (this.tetherState === DISCONNECTED) {
      ...
      if ((Date.now() - this.updatedLocalSelectionsAt) <= this.tetherBreakPeriod) return
    }
