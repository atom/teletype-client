...
class EditorProxy {
  ...
  constructor ({id, bufferProxy, selectionLayerIdsBySiteId, tethersByFollowerId, selections, router, siteId, tetherDisconnectWindow, didDispose, portal}) {
    this.id = id
    this.siteId = siteId
  ...
  bufferProxyDidUpdateMarkers (markerUpdates) {
    ...
    this.emitter.emit('did-update-remote-selections', selectionLayerIdsBySiteId)
  }
  ...
  receiveTetherUpdate (tetherUpdate) {
    this.portal.receiveTetherUpdate(tetherUpdate)
  }
}
