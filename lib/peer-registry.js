...
class PeerConnection {
  ...
  didReceiveEnvelope (envelope) {
    ...
    if (isRequest || isResponse) {
      const requestIdOffset = (metadata & metadataFlags.multipart) ? 5 : 1
      const requestId = envelope.readUInt32BE(requestIdOffset)
      const message = envelope.slice(requestIdOffset + 4)
