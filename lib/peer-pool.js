...
module.exports =
class PeerPool {
  constructor ({peerId, oauthToken, restGateway, pubSubGateway, fragmentSize, connectionTimeout, testEpoch}) {
    this.peerId = peerId
  ...
  async authenticate () {
    const {body, ok} = await this.restGateway.get('/identity', {
      headers: {'GitHub-OAuth-token': this.oauthToken}
    })

    if (ok) {
      this.peerIdentitiesById.set(this.peerId, body)
    } else {
      // TODO: Handle authentication failure and server errors
    }
  }
  ...
  getPeerConnection (peerId) {
    ...
    if (!peerConnection) {
      peerConnection = new PeerConnection({
        ...
        didError: this.didError.bind(this),
