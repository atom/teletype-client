...
class PeerPool {
  ...
  async waitForIncomingSignals () {
    const subscriptionPromise = this.pubSubGateway.subscribe(
      `/peers/${this.peerId}`,
      'signal',
      this.didReceiveSignal.bind(this)
    )
    const result = await Promise.race([subscriptionPromise, this.getNewTimeoutPromise()])
    if (result === TIMEOUT_SYMBOL) {
      subscriptionPromise.then((subscription) => subscription.dispose())
      throw new Errors.PubSubConnectionError('Listening for incoming connections timed out')
    }
  }

  async connectTo (peerId) {
    const peerConnection = this.getPeerConnection(peerId)
    const connectPromise = peerConnection.connect()
    const result = await Promise.race([connectPromise, this.getNewTimeoutPromise()])
    if (result === TIMEOUT_SYMBOL) {
      this.peerConnectionsById.delete(peerId)
      peerConnection.disconnect()
      throw new Errors.PeerConnectionError(`Connecting to peer ${peerId} timed out`)
    }
  }
