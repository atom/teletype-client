...
const NOOP = () => {}
const DEFAULT_TETHER_BREAK_PERIOD = 1000
const LOCAL_PROTOCOL_VERSION = 2

module.exports =
class RealTimeClient {
  constructor ({restGateway, pubSubGateway, connectionTimeout, tetherBreakPeriod, testEpoch, pusherKey, baseURL, didCreateOrJoinPortal}) {
    this.pusherKey = pusherKey
    this.restGateway = restGateway || new RestGateway({baseURL})
    this.pubSubGateway = pubSubGateway
    this.connectionTimeout = connectionTimeout || 5000
    this.tetherBreakPeriod = tetherBreakPeriod || DEFAULT_TETHER_BREAK_PERIOD
  ...
  async createPortal () {
    ...
    if (result.ok) {
      ...
      const portal = new Portal({
        ...
        connectionTimeout: this.connectionTimeout,
        tetherBreakPeriod: this.tetherBreakPeriod
      })
