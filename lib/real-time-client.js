...
class RealTimeClient {
  constructor ({restGateway, pubSubGateway, pusherKey, baseURL, didCreateOrJoinPortal}) {
  ...
  async initialize () {
    ...
    this.peerPool = new PeerPool({
      ...
      fragmentSize: 16 * 1024 * 1024 // 16KB
    })
    this.peerPool.initialize()
  }
  ...
  async joinPortal (id) {
    ...
    const portal = new Portal({id, peerPool: this.peerPool})
    await portal.join(hostPeerId)
    this.didCreateOrJoinPortal(portal)
