...
class Portal {
  ...
  siteDidLeave ({peerId, connectionLost}) {
    ...
    if (this.activeEditorProxy) {
      ...
      if (tether && siteId === tether.leaderId) {
        this.activeEditorProxy.unfollow()
      }
  ...
  activeEditorDidUpdateLocalSelections ({preserveFollowState}) {
    if (!preserveFollowState && this.activeEditorProxy.resolveFollowState() === FollowState.RETRACTED) {
      ...
      if (localCursorPosition && leaderPosition && !pointsEqual(localCursorPosition, leaderPosition)) {
        this.activeEditorProxy.setFollowState(FollowState.EXTENDED)
      }
    }
  ...
  activeEditorDidUpdateRemoteSelections (updatedSelectionLayersBySiteId) {
    ...
    if (updatedSelectionLayersBySiteId.has(leaderId)) {
      this.activeEditorProxy.retractOrDisconnectTether()
    }
  ...
  activeEditorDidUpdateText ({remote}) {
    if (this.activeEditorProxy.resolveFollowState() === FollowState.RETRACTED) {
      if (remote) {
        const leaderPosition = this.activeEditorProxy.resolveLeaderPosition()
        if (leaderPosition) {
          this.activeEditorProxy.delegate.updateTether(FollowState.RETRACTED, leaderPosition)
        }
      } else {
        this.activeEditorProxy.lastLocalUpdateAt = Date.now()
        this.activeEditorProxy.setFollowState(FollowState.EXTENDED)
      }
    }
