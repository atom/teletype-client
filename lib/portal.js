...
class Portal {
  ...
  async receiveEditorProxySwitch (senderSiteId, editorProxySwitch) {
    ...
    if (senderSiteId === this.resolveLeaderSiteId()) {
  ...
  activeEditorDidScroll () {
    const leaderPosition = this.resolveLeaderPosition()
    if (leaderPosition && !this.getLocalActiveEditorProxy().isPositionVisible(leaderPosition)) {
      this.unfollow()
    }
  }
  ...
  retractOrDisconnectTether () {
    ...
    const leaderPositionIsInvisible = (
      ...
      !localActiveEditorProxy.isPositionVisible(leaderPosition)
    )
