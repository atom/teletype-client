...
class Portal {
  ...
  async join () {
    ...
    if (activeEditorProxyMessage) {
      ...
      this.subscribeToEditorProxyChanges(activeEditorProxy)
  ...
  async receiveEditorProxySwitch (senderSiteId, editorProxySwitch) {
    ...
    if (leaderDidSwitch && editorProxy != previousLocalActiveEditorProxy) {
      const followState = this.resolveFollowState()
      if (followState === FollowState.EXTENDED) {
        this.retractOrDisconnectTether()
      }

      if (followState === FollowState.RETRACTED) {
        this.subscribeToEditorProxyChanges(editorProxy)
        this.delegate.updateTether(followState, editorProxy, this.resolveLeaderPosition())
      }
    }
  ...
  activeEditorDidUpdateRemoteSelections ({selectionLayerIdsBySiteId, initialUpdate}) {
    if (!initialUpdate) {
      ...
      const followState = this.resolveFollowState()
      if (followState === FollowState.EXTENDED && selectionLayerIdsBySiteId.has(leaderId)) {
        this.retractOrDisconnectTether()
      } else if (followState === FollowState.RETRACTED) {
        this.delegate.updateTether(followState, this.getLocalActiveEditorProxy(), this.resolveLeaderPosition())
      }
    }
  ...
  retractOrDisconnectTether () {
    ...
    const leaderPositionIsInvisible = (
      ...
      !localActiveEditorProxy.delegate.isPositionVisible(leaderPosition)
    )
