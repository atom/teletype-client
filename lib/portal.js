...
class Portal {
  constructor ({id, hostPeerId, siteId, peerPool, connectionTimeout, tetherDisconnectWindow}) {
    ...
    this.activeBufferProxySubscriptions = new CompositeDisposable()
  ...
  async setDelegate (delegate) {
    ...
    if (!this.isHost && this.activeEditorProxy) {
      await this.delegate.activateEditorProxy(this.activeEditorProxy)
    }
  }
  ...
  async join () {
    ...
    if (activeEditorProxyMessage) {
      ...
      this.activeBufferProxySubscriptions.dispose()
      this.activeBufferProxySubscriptions = new CompositeDisposable()
      this.activeBufferProxySubscriptions.add(this.activeEditorProxy.onDidScroll(this.activeEditorDidScroll.bind(this)))
      this.activeBufferProxySubscriptions.add(this.activeEditorProxy.onDidUpdateLocalSelections(this.activeEditorDidUpdateLocalSelections.bind(this)))
      this.activeBufferProxySubscriptions.add(this.activeEditorProxy.onDidUpdateRemoteSelections(this.activeEditorDidUpdateRemoteSelections.bind(this)))
      this.activeBufferProxySubscriptions.add(this.activeEditorProxy.bufferProxy.onDidUpdateText(this.activeEditorDidUpdateText.bind(this)))

      this.follow(1)
      this.delegate.activateEditorProxy(this.activeEditorProxy)
    }
  }
  ...
  activateEditorProxy (editorProxy) {
    ...
    if (editorProxy != this.activeEditorProxy) {
      ...
      this.activeBufferProxySubscriptions = new CompositeDisposable()
      if (editorProxy) {
        this.activeBufferProxySubscriptions.add(editorProxy.onDidScroll(this.activeEditorDidScroll.bind(this)))
        this.activeBufferProxySubscriptions.add(editorProxy.onDidUpdateLocalSelections(this.activeEditorDidUpdateLocalSelections.bind(this)))
        this.activeBufferProxySubscriptions.add(editorProxy.onDidUpdateRemoteSelections(this.activeEditorDidUpdateRemoteSelections.bind(this)))
        this.activeBufferProxySubscriptions.add(editorProxy.bufferProxy.onDidUpdateText(this.activeEditorDidUpdateText.bind(this)))
      }
  ...
  siteDidLeave ({peerId, connectionLost}) {
    ...
    if (this.activeEditorProxy) {
      const tether = this.activeEditorProxy.tethersByFollowerId.get(this.siteId)
      if (tether && siteId === tether.leaderId) {
        this.unfollow()
      }

      this.activeEditorProxy.tethersByFollowerId.delete(siteId)
  ...
  async receiveEditorProxySwitch (editorProxySwitch) {
    ...
    if (editorProxy != this.activeEditorProxy) {
      ...
      this.activeEditorProxy = editorProxy
      this.delegate.activateEditorProxy(this.activeEditorProxy)
      if (this.activeEditorProxy) this.follow(1)
    }
  }
  ...
  receiveTetherUpdate (tetherUpdate) {
    const oldResolvedLeaderId = this.activeEditorProxy.resolveLeaderSiteId()
    ...
    this.activeEditorProxy.tethersByFollowerId.set(followerSiteId, {leaderId: leaderSiteId, state: tetherState})

    const newResolvedLeaderId = this.activeEditorProxy.resolveLeaderSiteId()
    const newResolvedState = this.resolveFollowState()
    const isRetracted = newResolvedState === FollowState.RETRACTED

    if (isRetracted) {
      this.activeEditorProxy.hideSelections()
    } else {
      this.activeEditorProxy.showSelections()
    }
  ...
  activeEditorDidUpdateLocalSelections ({initialUpdate}) {
    ...
    if (!initialUpdate && this.activeEditorProxy.resolveFollowState() === FollowState.RETRACTED) {
      const localCursorPosition = this.activeEditorProxy.cursorPositionForSiteId(this.siteId)
      const leaderPosition = this.activeEditorProxy.resolveLeaderPosition()
  ...
  activeEditorDidUpdateRemoteSelections (updatedSelectionLayersBySiteId) {
    const leaderId = this.activeEditorProxy.resolveLeaderSiteId()
  ...
  activeEditorDidUpdateText ({remote}) {
    if (this.activeEditorProxy.resolveFollowState() === FollowState.RETRACTED) {
      if (remote) {
        const leaderPosition = this.activeEditorProxy.resolveLeaderPosition()
        if (leaderPosition) {
  ...
  activeEditorDidScroll () {
    const leaderPosition = this.activeEditorProxy.resolveLeaderPosition()
    if (leaderPosition && !this.activeEditorProxy.delegate.isPositionVisible(leaderPosition)) {
      this.unfollow()
    }
  }
  ...
  getFollowedSiteId () {
    return this.activeEditorProxy.getFollowedSiteId()
  }
  ...
  retractOrDisconnectTether () {
    ...
    if (!leaderPosition || followState === FollowState.DISCONNECTED) {
      ...
      if (this.activeEditorProxy.delegate.isPositionVisible(leaderPosition)) return
  ...
  setFollowState (newState, newLeaderId) {
    ...
    this.activeEditorProxy.tethersByFollowerId.set(this.siteId, {leaderId: newLeaderId, state: newState})
    ...
    if (oldState !== newState || oldLeaderId !== newLeaderId) {
      ...
      this.router.notify(`/editors/${this.activeEditorProxy.id}/updates`, editorProxyUpdateMessage.serializeBinary())
    }
  }
    ...
    for (let i = 0; i < activeSiteIds.length; i++) {
      ...
      if (siteId !== this.siteId) {
        ...
        if (this.resolveFollowState(siteId) === FollowState.RETRACTED) {
          ...
          position = this.activeEditorProxy.cursorPositionForSiteId(siteId)
        }
  ...
  resolveFollowState (followerId = this.siteId) {
    return this.activeEditorProxy.resolveFollowState(followerId)
  }

  resolveLeaderPosition (followerId = this.siteId) {
    return this.activeEditorProxy.resolveLeaderPosition(followerId)
  }

  resolveLeaderSiteId (followerId = this.siteId) {
    return this.activeEditorProxy.resolveLeaderSiteId(followerId)
  }
}
