...
class Portal {
  constructor ({id, hostPeerId, siteId, peerPool}) {
    ...
    if (this.isHost) {
      ...
      this.disposables.add(
        ...
        this.router.onTrack(`/portals/${id}/screen-share`, this.receiveScreenShareTrack.bind(this))
      )
    }
  ...
  addScreenShareTrack (track, stream) {
    this.router.broadcastTrack(`/portals/${this.id}/screen-share`, '', track, stream)
  }
  ...
  dispose () {
    ...
    this.textBuffersById.forEach((textBuffer) => {
      textBuffer.dispose()
    })
  ...
  receiveScreenShareTrack ({track}) {
    if (this.delegate) this.delegate.addScreenShareTrack(track)
  }
