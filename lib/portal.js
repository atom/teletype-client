...
class HostPortal {
  ...
  async close () {
    await this.dispose()
    await this.restGateway.delete(`/portals/${this.id}/sites/${this.siteId}`)
  }
  ...
  async subscribe () {
    this.subscriptions.push(await this.subscribeToSiteDidLoseConnectionEvents())
    this.subscriptions.push(await this.subscribeToSiteDidLeaveEvents())
  }

  subscribeToSiteDidLoseConnectionEvents () {
    return this.pubSubGateway.subscribe(
      `/portals/${this.id}`,
      'site-did-lose-connection',
      this.receiveSiteDidLoseConnection.bind(this)
    )
  }

  receiveSiteDidLoseConnection (message) {
  ...
  subscribeToSiteDidLeaveEvents () {
    return this.pubSubGateway.subscribe(
      `/portals/${this.id}`,
      'site-did-leave',
      this.receiveSiteDidLeave.bind(this)
    )
  }

  receiveSiteDidLeave (message) {
    let {siteId} = JSON.parse(message.text)
    siteId = parseInt(siteId)
...
class GuestPortal {
  ...
  async subscribe () {
    this.subscriptions.push(
      ...
      await this.subscribeToSiteDidLoseConnectionEvents(),
      await this.subscribeToSiteDidLeaveEvents()
    )
  }
  ...
  leave () {
    this.dispose()
    this.restGateway.delete(`/portals/${this.id}/sites/${this.siteId}`)
  }

  async receiveUpdate (message) {
    const previousSharedEditor = this.activeSharedEditor
    ...
    if (this.previousSharedEditor) this.previousSharedEditor.dispose()
  }
  ...
  async dispose () {
    if (!this.disposed) {
      ...
      if (this.activeSharedEditor) {
        await this.activeSharedEditor.dispose()
        this.activeSharedEditor = null
      }
