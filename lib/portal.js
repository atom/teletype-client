...
class Portal {
  ...
  async join () {
    ...
    this.activeTextEditor = await TextEditor.deserialize(response.getActiveTextEditor(), {
      router: this.router,
      siteId: this.siteId,
  ...
  async receiveUpdate ({message}) {
    ...
    let textBuffer = this.textBuffersById.get(textBufferId)
    if (!textBuffer) {
      ...
      textBuffer = TextBuffer.deserialize(textBufferMessage, {
        router: this.router,
        siteId: this.siteId
      })
