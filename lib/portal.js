...
class Portal {
  ...
  async receiveEditorProxySwitch (editorProxySwitch) {
    ...
    if (bufferProxyId && !this.bufferProxiesById.has(bufferProxyId)) {
      const response = await this.router.request({recipientId: this.hostPeerId, channelId: `/buffers/${bufferProxyId}`})
    ...
    if (editorProxyId && !this.editorProxiesById.has(editorProxyId)) {
      const response = await this.router.request({recipientId: this.hostPeerId, channelId: `/editors/${editorProxyId}`})
      if (!response.ok) return
