...
class HostPortal {
  constructor ({restGateway, pubSubGateway, heartbeatIntervalInMilliseconds}) {
    ...
    this.queue = new OperationQueue() // TODO Consider a more descriptive variable name (e.g., outboundQueue or outboundOperationQueue)
  ...
  setActiveSharedEditor (sharedEditor) {
    ...
    if (enqueuedOperation && enqueuedOperation.id === id) {
      enqueuedOperation.data = sharedEditorId
    } else {
      this.queue.push({
        id,
        data: sharedEditorId,
        fn: this.sendActiveSharedEditorId.bind(this)
      })
    }
  }
  ...
  sendActiveSharedEditorId (sharedEditorId) {
    this.restGateway.put(`/portals/${this.id}`, {sharedEditorId})
  }
  ...
  async dispose () {
    if (!this.disposed) {
      ...
      if (this.queue) await this.queue.dispose()
...
class GuestPortal {
  constructor ({id, restGateway, pubSubGateway, heartbeatIntervalInMilliseconds}) {
    this.restGateway = restGateway
    this.pubSubGateway = pubSubGateway
    this.queue = null // TODO Consider a more descriptive variable name (e.g., outboundQueue or outboundOperationQueue)
    this.heartbeatIntervalInMilliseconds = heartbeatIntervalInMilliseconds
    this.heartbeat = null
    this.id = id
    this.siteId = null
