...
class Portal {
  ...
  getFollowedSiteId () {
    if (this.resolveFollowState() === FollowState.DISCONNECTED) {
      ...
      return this.activeEditorProxy.tethersByFollowerId.get(this.siteId).leaderId
    }
  }
  ...
  resolveLeaderPosition (followerId = this.siteId) {
    return this.activeEditorProxy.cursorPositionForSiteId(this.resolveLeaderSiteId(followerId))
  }
  ...
  resolveFollowState (followerId = this.siteId) {
    ...
    if (followerId === leaderId) {
      ...
      return this.activeEditorProxy.tethersByFollowerId.get(followerId).state
    }
  }

  resolveLeaderSiteId (followerId = this.siteId) {
    const tether = this.activeEditorProxy.tethersByFollowerId.get(followerId)
    if (!tether) return followerId

    const visitedSiteIds = new Set([followerId])
    let leaderId = tether.leaderId

    let nextTether = this.activeEditorProxy.tethersByFollowerId.get(leaderId)
    while (nextTether && nextTether.state === FollowState.RETRACTED) {
      if (visitedSiteIds.has(leaderId)) {
        ...
        nextTether = this.activeEditorProxy.tethersByFollowerId.get(leaderId)
      }
    }
