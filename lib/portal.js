...
class Portal {
  ...
  async join () {
    ...
    if (activeEditorProxyMessage) {
      ...
      this.activeEditorProxySubscriptions = new CompositeDisposable()
      this.activeEditorProxySubscriptions.add(activeEditorProxy.onDidScroll(this.activeEditorDidScroll.bind(this)))
      this.activeEditorProxySubscriptions.add(activeEditorProxy.onDidUpdateLocalSelections(this.activeEditorDidUpdateLocalSelections.bind(this)))
      this.activeEditorProxySubscriptions.add(activeEditorProxy.onDidUpdateRemoteSelections(this.activeEditorDidUpdateRemoteSelections.bind(this)))
      this.activeEditorProxySubscriptions.add(activeEditorProxy.bufferProxy.onDidUpdateText(this.activeEditorDidUpdateText.bind(this)))
  ...
  activateEditorProxy (editorProxy) {
    if (editorProxy != this.getLocalActiveEditorProxy()) {
      this.unfollow()
      this.activeEditorProxiesBySiteId.set(this.siteId, editorProxy)
      this.didChangeActiveEditorProxy(editorProxy)
      this.broadcastEditorProxySwitch(editorProxy)
    }
  }
  ...
  async receiveEditorProxySwitch (senderSiteId, editorProxySwitch) {
    ...
    if (leaderDidSwitch && isRetracted && editorProxy != previousLocalActiveEditorProxy) {
      this.didChangeActiveEditorProxy(editorProxy)
  ...
  didChangeTetherState ({oldResolvedState, oldResolvedLeaderId, newResolvedState, newResolvedLeaderId}) {
    ...
    if (newResolvedState === FollowState.RETRACTED) {
      // TODO Add test to prove the correctness of this behavior
      this.editorProxiesById.forEach((editorProxy) => {
