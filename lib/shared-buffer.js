...
const DocumentReplica = require('@atom-team/tachyon')
...
class SharedBuffer {
  static async create ({delegate, restGateway, pubSubGateway}) {
    ...
    const {id} = await restGateway.post('/shared-buffers', {operations: [msgpack.encode(operation).toString('base64')]})
  ...
  applyRemoteOperations (operations) {
    operations = operations.map((op) => msgpack.decode(new Buffer(op, 'base64')))
    ...
    if (this.queuedOutboundOperations) {
      ...
      await this.restGateway.post(
        `/shared-buffers/${this.id}/operations`,
        {
          ...
          operations: operations.map((op) => msgpack.encode(op).toString('base64'))
        }
      )
