...
class TextBuffer {
  ...
  constructor ({id, uri, text, operations, router, siteId}) {
    ...
    this.replica = new DocumentHistory(siteId)
    ...
    if (text) {
      ...
      const operations = this.replica.setTextInRange(position, position, text)
  ...
  setDelegate (delegate) {
    ...
    if (this.siteId !== 1 && this.delegate) {
      ...
      this.delegate.updateMarkers(this.replica.getMarkers())
    }
  }
  ...
  setTextInRange (oldStart, oldEnd, newText) {
    const operations = this.replica.setTextInRange(oldStart, oldEnd, newText)
  ...
  updateMarkers (markerUpdatesByLayerId) {
    const operations = this.replica.updateMarkers(markerUpdatesByLayerId)
    this.receivedOperations.push(...operations)
  ...
  integrateOperations (operations) {
    this.receivedOperations.push(...operations)
    const {textUpdates, markerUpdates} = this.replica.integrateOperations(operations)
    if (this.delegate) {
      this.delegate.updateText(textUpdates)
      this.delegate.updateMarkers(markerUpdates)
    }
  }
}
