...
class StarOverlayNetwork {
  constructor ({id, peerPool, isHub}) {
    ...
    this.usersByPeerId = this.getDefaultUsersByPeerId()
  ...
  connectTo (hubId) {
    ...
    return new Promise(async (resolve) => {
      ...
      const joinRequest = new NetworkMessage.StarJoinRequest()
      joinRequest.setSenderId(this.getPeerId())
      const networkMessage = new NetworkMessage()
      networkMessage.setStarJoinRequest(joinRequest)
  ...
  getMembers () {
    ...
    this.usersByPeerId.forEach((user, peerId) => {
  ...
  getDefaultUsersByPeerId () {
    const thisPeerId = this.getPeerId()
    const thisUser = this.peerPool.getUser(this.getPeerId())
    return new Map([[thisPeerId, thisUser]])
  }
  ...
  receiveJoinRequest (rawMessage, joinRequest) {
    ...
    const networkMembers = Array.from(this.getMembers()).map((member) =>
      serializeNetworkMember(member)
    )
    ...
    const user = this.peerPool.getUser(senderId)
    const member = Object.assign({}, user, {peerId: senderId})
    joinNotification.setMember(serializeNetworkMember(member))
    ...
    this.usersByPeerId.set(senderId, this.peerPool.getUser(senderId))
  ...
  receiveJoinResponse (joinResponse) {
    ...
    for (let i = 0; i < networkMembers.length; i++) {
      ...
      this.usersByPeerId.set(member.getPeerId(), {username: member.getUsername()})
    }
...
function serializeNetworkMember ({peerId, username}) {
  const networkMember = new NetworkMember()
  networkMember.setPeerId(peerId)
  networkMember.setUsername(username)
