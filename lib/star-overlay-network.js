...
class StarOverlayNetwork {
  ...
  receiveJoinRequest (rawMessage, connectionMessage) {
    ...
    if (this.isHub) {
      ...
      joinResponseMessage.setMemberIdsList(Array.from(this.members))
      const networkMessage = new NetworkMessage()
      networkMessage.setNetworkId(this.id)
      networkMessage.setStarJoinResponse(joinResponseMessage)
      this.send(senderId, networkMessage.serializeBinary())
  ...
  receiveJoinResponse (joinResponseMessage) {
    ...
    const memberIds = joinResponseMessage.getMemberIdsList()
