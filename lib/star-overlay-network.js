...
class StarOverlayNetwork {
  constructor ({id, peerPool, isHub}) {
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      peerPool.onMediaTrack(this.handleMediaTrack.bind(this))
    )
  }
  ...
  broadcastMediaTrack (metadata, track, stream) {
    const starMediaTrackBroadcast = new NetworkMessage.StarMediaTrackBroadcast()
    starMediaTrackBroadcast.setSenderId(this.peerPool.peerId)
    starMediaTrackBroadcast.setTrackId(track.id)
    const networkMessage = new NetworkMessage()
    networkMessage.setStarMediaTrackBroadcast(starMediaTrackBroadcast)
    networkMessage.setNetworkId(this.id)
    const rawNetworkMessage = networkMessage.serializeBinary()
    ...
    if (this.isHub) {
      this.spokes.forEach((peerId) => {
        ...
        this.peerPool.addMediaTrack(peerId, track, stream)
      })
    } else {
  ...
  receive ({senderId, message}) {
    ...
    } else if (networkMessage.hasStarMediaTrackBroadcast()) {
      this.receiveMediaTrackBroadcast(message, networkMessage.getStarMediaTrackBroadcast())
    }
  }
  ...
  receiveMediaTrackBroadcast (rawMessage, mediaTrackBroadcast) {
    console.log('receiveMediaTrackBroadcast');
  }
  ...
  handleMediaTrack ({senderId, track}) {
    console.log('got track', senderId, track);
  }
}
