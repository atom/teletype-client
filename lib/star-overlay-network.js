...
class StarOverlayNetwork {
  ...
  broadcastTrack (metadata, track, stream) {
    ...
    starTrackBroadcast.setMetadata(convertToProtobufCompatibleBuffer(metadata))
    const networkMessage = new NetworkMessage()
  ...
  receiveTrackBroadcast (rawNetworkMessage, trackBroadcast) {
    ...
    const metadata = convertToProtobufCompatibleBuffer(trackBroadcast.getMetadata())
    this.trackDataByTrackId.set(trackId, {senderId, metadata, rawNetworkMessage})

    if (this.isHub) {
      const track = this.peerPool.getIncomingTrack(senderId, trackId)
      if (track) this.handleTrack({track})
      this.forwardBroadcast(senderId, rawNetworkMessage)
    } else {
      const track = this.peerPool.getIncomingTrack(this.hubId, trackId)
      if (track) this.handleTrack({track})
    }
  }
