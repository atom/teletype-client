...
class StarOverlayNetwork {
  constructor ({id, peerPool, isHub}) {
    ...
    this.membersByPeerId = new Map()
    this.membersByPeerId.set(this.getPeerId(), this.peerPool.getUser(this.getPeerId())) // TODO Consider moving this to a fn, since same this logic exists in multiple places in this class
  ...
  disconnect () {
    ...
    this.membersByPeerId.clear()
    this.membersByPeerId.set(this.getPeerId(), {
  ...
  getMembers () {
    ...
    this.membersByPeerId.forEach((user, peerId) => {
  ...
  receiveJoinRequest (rawMessage, joinRequest) {
    ...
    this.membersByPeerId.set(senderId, this.peerPool.getUser(senderId))
  ...
  receiveJoinResponse (joinResponse) {
    ...
    for (let i = 0; i < networkMembers.length; i++) {
      ...
      this.membersByPeerId.set(member.getPeerId(), {
  ...
  handleDisconnection (peerId, connectionLost) {
    if (peerId === this.hubId) {
      ...
      this.membersByPeerId.delete(peerId)
    }
