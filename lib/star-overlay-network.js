...
const {NetworkMessage, PeerIdentity} = require('./real-time_pb')
...
class StarOverlayNetwork {
  constructor ({id, peerPool, isHub, connectionTimeout}) {
    ...
    const peerIdentity = this.peerPool.getPeerIdentity(this.getPeerId())
    this.memberIdentitiesById = new Map()
    this.clearOtherMembers()
  ...
  getMemberIds () {
    return Array.from(this.memberIdentitiesById.keys())
  }
  ...
  receiveJoinRequest (rawMessage, connectionMessage) {
    ...
    this.memberIdentitiesById.forEach((identity, peerId) => memberIdentitiesByIdMessage.set(peerId, serializePeerIdentity(identity)))
  ...
  receiveLeaveNotification (rawMessage, leaveNotification) {
    ...
    this.peerDidLeave(memberId, leaveNotification.getConnectionLost())
  }

  peerDidLeave (peerId, connectionLost) {
    if (peerId === this.hubId) {
      this.hubId = null
      this.state = 'disconnected'
      this.clearOtherMembers()
    } else {
      this.memberIdentitiesById.delete(peerId)
    }
  ...
  clearOtherMembers () {
    ...
    this.memberIdentitiesById.set(this.getPeerId(), this.peerPool.getPeerIdentity(this.getPeerId()))
  }
}
